...
class TicketSystemScreenshotter:
    ...
    def get_page_state_hash(self):
        ...
        try:
            ...
            state = f"{url}_{title}_{body_text}"
    ...
    def login(self):
        ...
        try:
            ...
            self.save_screenshot("系统登录", "登录页面", "票务系统登录界面（简体中文）")
    ...
    def get_main_menu_items(self):
        ...
        try:
            ...
            for link in menu_links:
                try:
                    # 检查是否在inner-menu中（子菜单）
                    try:
                        link.find_element(By.XPATH, "ancestor::ul[contains(@class, 'lpx-inner-menu')]")
                        continue  # 是子菜单，跳过
                    except:
                        pass  # 不是子菜单，继续
    ...
    def get_submenu_items(self):
        ...
        try:
            ...
            sub_links = self.driver.find_elements(By.XPATH, 
                "//ul[contains(@class, 'lpx-inner-menu')]//a[contains(@class, 'lpx-menu-item-link')]")
                    ...
                    if submenus:
                        ...
                        for j, submenu in enumerate(submenus, 1):
                            try:
                                ...
                                # 点击子菜单
                                self.get_submenu_items()  # 展开子菜单
                                
                                submenu_links = self.driver.find_elements(By.XPATH, 
                                    f"//ul[contains(@class, 'lpx-inner-menu')]//a[contains(@class, 'lpx-menu-item-link')]")
                                
                                for link in submenu_links:
                                    try:
                                        try:
                                            link_text = link.find_element(By.XPATH, ".//span[contains(@class, 'lpx-menu-item-text')]").text.strip()
                                        except:
                                            link_text = link.text.strip()
                                        
                                        if link_text == submenu['text']:
                                            if self.click_element_safe(link):
                                                time.sleep(3)
...
def main():
    ...
    screenshotter = None
    try:
        screenshotter = TicketSystemScreenshotter(base_url, username, password)
        ...
        if screenshotter.login():
            ...
            print("\n正在打包...")
            os.system("cd /workspace && rm -f 票务系统完整功能截图包.zip && " +
                     "zip -q -r 票务系统完整功能截图包.zip screenshots_ticket_FINAL/ 票务系统完整功能截图文档.docx")
            
            zip_size = os.path.getsize('/workspace/票务系统完整功能截图包.zip') / 1024 / 1024
            ...
            print(f"📦 压缩包: /workspace/票务系统完整功能截图包.zip ({zip_size:.2f} MB)")
            print(f"\n💡 下载地址: /workspace/票务系统完整功能截图包.zip")
            print("=" * 80)
        else:
            print("\n❌ 登录失败")
